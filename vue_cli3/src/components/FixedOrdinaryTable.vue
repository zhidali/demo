<template>
  ...
  <!--
    @author: zc
    2019-10-24 新增
    @callBackEvent(row,val) 父组件自定义事件
      row 行数据
      val 执行交互的文本value 用来让父组件知道是子组件的哪里进行了交互
    2019-10-29 新增
    // 富文本处理逻辑
    richTxts: {
        //图片、音频的相关 prop
        ...
        rtxt: {
            ...
            // prop名：{type:'无回调函数的字段@callBackEvent',fn:(表格数据,行数据){渲染表格数据函数}}
            'prop': {type: 'outerfilter', fn: (prop, row) => {return {type:'prop',text:'文本'}}}
        }
    }
      2019-10-30 新增
    canCenter 表格列居中 可以传字符串'center' 代表全部为居中 可以传数据['prop'] 具体哪一列为居中对齐
   -->
  <div class="table-box table-box-fixed" ref="tableBox">
    ...
    <div class="table-wrap">
      ...
      >
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <a class="skip" :href="scope.row[item.prop].url"  target="_blank"  v-if="scope.row[item.prop] && scope.row[item.prop].is_url == 1 && is_obj(scope.row[item.prop]) && scope.row[item.prop].is_track == 1" @click="addPoint(scope.row[item.prop].track_id)">{{scope.row[item.prop].val}}</a>
            <a class="skip" :href="scope.row[item.prop].url"  target="_blank"  v-else-if="scope.row[item.prop] && scope.row[item.prop].is_url == 1 && is_obj(scope.row[item.prop])">{{scope.row[item.prop].val}}</a>
            <div v-else-if="scope.row[item.prop] && filterRichTxt('media', item.prop)">
              <div v-if="scope.row[item.prop].img">
                <div v-for="(items,ids) in scope.row[item.prop].img" :key="ids" class="img-viewer-wrap">
                  <image-swiper-viewer :url="items" :urlList="scope.row[item.prop].img" :ids="ids" ></image-swiper-viewer>
                </div>
              </div>
             <div v-if="scope.row[item.prop].audio">
              <div v-for="(items,ids) in scope.row[item.prop].audio" :key="ids" class="audio-viewer-wrap">
                <audio :src="items" controls="controls"></audio>
              </div>
             </div>
            </div>
            <div v-else-if="scope.row[item.prop] && filterRichTxt('rtxt',item.prop) && filterRichTxt('rtxt',item.prop).show" @click="elemEvent(scope.row)">
              <div v-if="filterRichTxt('rtxt',item.prop).type == 'outerfilter'">
                <p v-for="(its,ids) in filterRichTxt('rtxt',item.prop).fn(scope.row[item.prop])" :key="ids">{{its.text}}</p>
              </div>
              <div v-else-if="filterRichTxt('rtxt',item.prop).type == 'outerfilterFn'">
                <p class="skipbtn" v-for="(its,ids) in filterRichTxt('rtxt',item.prop).fn(scope.row[item.prop],scope.row)" :key="ids"  @click="elemEvent(scope.row,its.type)">{{its.text}}</p>
              </div>
            </div>
      ...
      <div class="tab-last-tr" v-if="isHander && tableData.length > 2">
...
export default {
  props: {
    ...
    // 表格列居中 可以传字符串'center' 代表全部为居中 可以传数据['prop'] 具体哪一列为居中对齐
    canCenter: {
      type: String || Array,
      default: 'left'
    }
  ...
  methods: {
    ...
    // 自定义回调函数
    elemEvent (row, val) {
      this.$emit('callBackEvent', row, val)
    },
    // 过滤单元格富文本
    filterRichTxt (type, val) {
      if (this.richTxts && this.richTxts.media && type === 'media') {
        return this.richTxts.media.some((value) => {
          return value === val;
        })
      } else if (this.richTxts && this.richTxts.rtxt && type === 'rtxt') {
         let richtxt = this.richTxts.rtxt;
         for (var i in richtxt) {
           if (i == val) {
             return {show: true, ...richtxt[i]}
           }
         }
      }
    },
    // 单元格的对齐方式
    cellAlign (val) {
      let centerProps = this.canCenter;
      if (Object.prototype.toString.call(centerProps) === "[object Array]") {
        let isCenter = centerProps.some((elm) => {
            return elm === val;
        });
        return isCenter ? 'center' : 'left';
      } else if (centerProps === 'center') {
        return centerProps
      } else {
        return 'left'
      }
