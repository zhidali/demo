          ...
          <ul class="sub-list" v-loading="normalLoading">
            <li v-for="(school, index) in aroundList" :key="index" v-if="school.list.length > 0">
              <div class="result-tab-hd">
                <div class="tab-hd-img"><img src="../../../../static/images/sreachTool/local_icon.png"></div>
...
export default {
  ...
  mounted () {
    ...
    CommonMethods.setTrack('4170', this.trackMessages, trackObj, this.backendApi)
    // 通知当前周报tab
    if (this.notAround) {
  ...
  methods: {
    ...
    chooseItem (para, paraIndex, paraName) {
      ...
      if (paraName) {
        ...
        CommonMethods.setTrack('3920', this.trackMessages, trackObj, this.backendApi)
        // 周边
        // 给地图组件传值
