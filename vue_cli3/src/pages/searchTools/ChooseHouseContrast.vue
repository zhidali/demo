        ...
        methods: {
            ...
            async getCity() {
                try {
                    ...
                    if (data.code === 0) {
                        ...
                        this.checkCityConBase = commonMethod.deepClone(this.checkCityCon);
            ...
            changeCityId(item, majorId, type) {
                ...
                if (notFilArr.length > 0) {
                    ...
                    this.cityTotal = commonMethod.deepClone(arr);
                }
                if (item.state) {
                    this.checkCityCon.push({
                        id: item.id,
            ...
            closeCityPop(type) {
                // 取消重置已选择的
                if (type === 0) {
                    this.checkCityCon = commonMethod.deepClone(this.checkCityConBase);
                    this.cityTotal = commonMethod.deepClone(this.cityTotalBase);
                    // 选中主城的top值
                    this.citySrollTop = document.querySelector('.current-major-active') ? document.querySelector('.current-major-active').offsetTop - 4 : 0;
                } else {
                    // 确定更新数据
                    ...
                    this.cityTotalBase = commonMethod.deepClone(this.cityTotal);
                }
                document.getElementsByClassName('major-con')[0].scrollTop = this.citySrollTop;
